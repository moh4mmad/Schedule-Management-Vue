<template>
  <div>
    <CRow v-can="'employee-edit'">
      <CCol md="12">
        <CCard>
          <CCardHeader>
            <CRow>
              <CCol sm="5">
                <h4 id="traffic" class="card-title mb-0">Update employee</h4>
              </CCol>
          ...
          <form name="form" @submit.prevent="updateEmployee">
            <CCardBody>
              <CInput
                v-model="user.name"
                type="text"
...
export default {
  ...
  methods: {
    ...
    updateEmployee() {
      let id = this.$route.params.id;
      Service.Put("employees/" + id, this.user).then((response) => {
        if (response.status == 200) {
